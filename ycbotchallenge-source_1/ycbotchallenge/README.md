...
Chat is the source of truth for money and upgrade outcomes. EnchantedMC formats parsed out of the box: success `You have unlocked a new sword level for 144.86B!` (the paid amount is debited from the local balance immediately), fail `You need 96.56B Money...` (the amount is the TOTAL price — affordability is `bal >= remaining`), zone fails split across two lines (`...next stage.` then `You need X Money.`). `/bal` replies (`- Money: 45.22B`) are only accepted within 8 s of a `/bal` send. Sidebar lines are stripped of § codes before matching. A response latch bounds any pattern-blind spot to one send per 75 s, and unrecognized post-send lines are raw-logged as `upgrade_response_raw` for one-shot pattern tuning. Config pattern lists auto-migrate (`configVersion`).
